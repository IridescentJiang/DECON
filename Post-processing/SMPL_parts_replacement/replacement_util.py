...
class SMPLX():
    ...
    def __init__(self):
        ...
        self.current_dir = "./smpl_data"  # new smplx file in ECON folder
        ...
        self.smplx_vertex_lmkid_path = osp.join(
            self.current_dir, "smplx_vertex_lmkid.npy"
        )
...
def part_removal(full_mesh, part_mesh, thres, device, smpl_obj, region, clean=True):
    ...
    if region == "hand":
        ...
        remove_mask = torch.logical_and(
            remove_mask,
            torch.tensor(full_lmkid >= 8).type_as(remove_mask).unsqueeze(0)
        )
        ...
        remove_mask = torch.logical_and(
            remove_mask,
            torch.tensor(full_lmkid >= 5).type_as(remove_mask).unsqueeze(0)
        )
...
def poisson(mesh, soup_path, result_path, depth=10, decimation=True):
    pcd_path = soup_path[:-4] + "_soups.ply"
    ...
    largest_mesh.export(result_path)

    if decimation:
        # mesh decimation for faster rendering
        low_res_mesh = largest_mesh.simplify_quadratic_decimation(100000)
        return low_res_mesh
    else:
        return largest_mesh
