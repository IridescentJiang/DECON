...
from src.dataset.multi_view_dataset import MultiViewDataset
...
class Net(nn.Module):
    ...
    def forward(
        ...
        if self.semantic_guider is not None and cond_semantic_imgs is not None: # normal guider和normal_img都不为None（normal_img为None表示cfg drop）
            semantic_fea = self.semantic_guider(cond_semantic_imgs.to(device="cuda")) # normal_guider和tensor都是float32，出来的fea是float16
        else:
            semantic_fea = None
        if self.normal_guider is not None and cond_normal_imgs is not None:
            normal_fea = self.normal_guider(cond_normal_imgs.to(device="cuda"))
        else:
            normal_fea = None
...
def log_validation(
    ...
    subjects = ["0080", "0125", "2438", "2440"]
    validation_dir = "/apdcephfs_cq10/share_1330077/hexu/NVS/data_rendering/THuman2/ortho_6" ##debug
...
def main(cfg, cfg_path):
    ...
    if (cfg.unet_additional_kwargs.motion_module_kwargs.temporal_position_encoding_type == "RPE"):
        cfg.unet_additional_kwargs.motion_module_kwargs.update(
            {"temporal_position_encoding_max_len": cfg.data.n_sample_frames}
            ) # 序列长用于确认rpe长度
    ...
    denoising_unet = UNet3DConditionModel.from_pretrained_2d(
        ...
        load_pe=False, # 现在的motion module的pe不吻合，所以不加载;推理要加载
    ).to(device="cuda")
    ...
    denoising_unet.load_state_dict(
        ...
        strict=False, # denoising unet有新增的time层 所以false
    )
    ...
    if semantic_guider is not None:
        semantic_guider.requires_grad_(False)
    if normal_guider is not None:
        normal_guider.requires_grad_(False)
    
    # Set motion module learnable
    for name, param in denoising_unet.named_parameters():
        if (
            "motion_modules" not in name
            and "transformer_blocks" in name 
            and ("attn0" in name or "norm0" in name)
            ): # 原本unet中的transformer里的norm0和attn0 
               #(因为motion_modul里面也有transformer_blocks, 所以先把motion module排除)
            param.requires_grad = True
        if "motion_modules" in name:
            param.requires_grad = True
    # for name, module in denoising_unet.named_modules():
    #     if "transformer_blocks" in name:
    #         if "attn0" in name or "norm0" in name: # 只训练(multi view)self attn
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=cfg.data.train_bs, shuffle=True, num_workers=4 # debug
    )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                # 过vae
                tgt_rgb_imgs = batch["tgt_rgb_imgs"].to(weight_dtype)
                tgt_normal_imgs = batch["tgt_normal_imgs"].to(weight_dtype)
                # pixel_values_vid = batch["pixel_values_vid"].to(weight_dtype)
                with torch.no_grad():
                    video_length = tgt_rgb_imgs.shape[1]
                    tgt_rgb_imgs = rearrange(tgt_rgb_imgs, "b f c h w -> (b f) c h w")
                    tgt_rgb_latents = vae.encode(tgt_rgb_imgs).latent_dist.sample()
                    tgt_rgb_latents = rearrange(tgt_rgb_latents, "(b f) c h w -> b c f h w", f=video_length)
                    tgt_rgb_latents = tgt_rgb_latents * 0.18215
                    
                    tgt_normal_imgs = rearrange(tgt_normal_imgs, "b f c h w -> (b f) c h w")
                    tgt_normal_latents = vae.encode(tgt_normal_imgs).latent_dist.sample()
                    tgt_normal_latents = rearrange(tgt_normal_latents, "(b f) c h w -> b c f h w", f=video_length)
                ...
                if cfg.noise_offset > 0.0:
                    rgb_noise += cfg.noise_offset * torch.randn(
                        (rgb_noise.shape[0], rgb_noise.shape[1], rgb_noise.shape[2], 1, 1),
                        device=rgb_noise.device,) # 不同bc加不一样的offset，但是像素点之间offset相同
                    normal_noise += cfg.noise_offset * torch.randn(
                        (normal_noise.shape[0], normal_noise.shape[1], normal_noise.shape[2], 1, 1),
                ...
                ## 准备camera
                cameras = batch["tgt_camera"].to(weight_dtype) # bs, f, 4, 4
                bs, f, _, _ = cameras.shape
                if cfg.camera_use_radius:
                    cameras = cameras.reshape(bs, f, -1) # bs, f, 16
                else: # 走这里
                    cameras = cameras[:, :, :3, :3].reshape(bs, f, -1)  # bs f 9
                # ref_camera
                ...
                if not drop_smplx:
                    cond_semantic_imgs = batch["cond_semantic_imgs"].transpose(1, 2) # (bs, 3, f, 512, 512)
                    cond_normal_imgs = batch["cond_normal_imgs"].transpose(1, 2)  # (bs, 3, f, 512, 512)                 
                else:
                    cond_semantic_imgs = None
                    cond_normal_imgs = None

                ## 准备ref rgb/normal 包括vae和clip
                uncond_fwd = random.random() < cfg.uncond_ratio
                # uncond_fwd = False #debug
                clip_image_list = []
                ref_rgb_list = []
                ref_normal_list = []
                for batch_idx, (ref_rgb_img, ref_normal_img, clip_img) in enumerate(
                    ...
                    ref_rgb_list.append(ref_rgb_img)
                    ref_normal_list.append(ref_normal_img)

                with torch.no_grad():
                    ref_rgb = torch.stack(ref_rgb_list, dim=0).to(dtype=vae.dtype, device=vae.device)
                    ref_rgb_latents = vae.encode(ref_rgb).latent_dist.sample()  # (bs, d, 64, 64) d=4
                    ref_rgb_latents = ref_rgb_latents * 0.18215
                    
                    ref_normal = torch.stack(ref_normal_list, dim=0).to(dtype=vae.dtype, device=vae.device)
                    ...
                    if accelerator.is_main_process:
                        ...
                        for sample_id, sample_dict in enumerate(sample_dicts):
                            ...
                            with TemporaryDirectory() as temp_dir:
                                ...
                                save_videos_grid(vid, out_file, n_rows=4, fps=5)
        ...
        if (
            ...
            # 把整个denoising_unet保存了 包括了motion module
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/train/stage2_union.yaml") ##debug
