...
@ torch.no_grad()
def save_optimed_video(path, rgb_video, normal_video, smpl_normals, smpl_semantics=None):
    # input:
    ...
    merge = (rgb_video + smpl_normals)/2.0
    diff = normal_video - smpl_normals
    if smpl_semantics is not None:
        smpl_semantics = smpl_semantics.detach().cpu()
        videos = torch.cat([rgb_video, normal_video, smpl_normals, smpl_semantics, merge, diff], dim=0)
        save_videos_grid(videos.repeat(1,1,2,1,1), path, n_rows=3, fps=10)
    else:
        videos = torch.cat([rgb_video, normal_video, smpl_normals, merge, diff], dim=0)
        save_videos_grid(videos.repeat(1,1,2,1,1), path, n_rows=3, fps=10)
...
class SMPLEstimator:
    ...
    def estimate_smpl(self, img_pil): # 估计smplx参数
    ...
    def smpl_forward(self,
        ...
        smpl_verts = (smpl_verts + optimed_trans) * scale * torch.tensor([1.0, -1.0, -1.0]).to(self.device) # 是把1 -1 -1这个tensor放到cuda 前面已经在cuda
        smpl_joints = (smpl_joints + optimed_trans) * scale * torch.tensor([-1.0, 1.0, -1.0]).to(self.device) # 145 3
        smpl_joints_3d = smpl_joints[:, self.smpl_data.smpl_joint_ids_45_pixie, :] # [-1,1]范围
...
class SMPLRenderer:
    ...
    def set_cameras(self, azim_list, elev_list):
        ...
        self.R = R.to(self.device) # 可以用于world_space到screen_space的转换
    ...
    def load_mesh(self, verts, faces, reverse_normal=False):
        ...
        Args:
            ...
            reverse_normal: 这是因为neus重建的normal似乎法线是朝里的,需要反向一下
        """
        ## 准备normal mesh
        self.mesh = Meshes(verts, faces).to(self.device) # 只走这里
        # normal着色
    ...
    def get_image(self, type="normal", bg="black"):
        ...
        # 渲染semantic的时候加载不同的mesh 其余都用相同的
        if type == "semantic":
        ...
        if type == "depth":
        ...
        elif type == "normal": # semantic和normal都用相同的渲染器 只是不同纹理的mesh
            images = self.renderer(current_mesh.extend(len(self.cameras)))
            # images = self.renderer(current_mesh)  ##debug
            ...
            images = self.renderer(current_mesh.extend(len(self.cameras))) # B H W C=4 单通道的颜色索引 前三个C维度是相同的 最后是透明度
            images = images[:,:,:,0] # B H W 索引
    ...
    def render_mask(self, bg="black"): # 可微mask
        return self.get_image(type="mask", bg=bg) # bg默认是black
    
    def render_normal(self, bg="black"): # 可微normal,world_space,用于和branch分支算loss
        # output:
        #     normals: tensor B,C=3,H,W [0,1]
        images = self.get_image(type="normal", bg=bg) # world B,C=4,H,W [0,1]范围 和渲染数据预处理的结果是对应的 
        images = images[:,:3,:,:]
        return images
    
    
    def project_joints(self, points_3d):
        # input: points_3d tensor B=1个3d模型,N点,C=3 [-1,1]
        # output: points_2d tensor B=24张角度图,N,C=2 [0,1] 和mediapipe识别出的对齐
    ...
    @torch.no_grad()
    def render_normal_screen_space(self, bg="black", return_mask=False): # 不可微normal,screen_space,用于nvs条件
        # output:
        #     normals: tensor B,C,H,W [0,1]
        #     masks: tensor B,H,W {0,1}
        images = self.get_image(type="normal", bg=bg).detach() # B,C=4,H,W [0,1]范围 和渲染数据预处理的结果是对应的
        normals = images[:,:3,:,:] * 2 - 1 # B,3,H,W [-1,1]物理空间 注意背景被变成了(-1,-1,-1) 前景则是长度归1的向量
        masks = images[:,3,:,:].unsqueeze(1) # B,1,H,W
        masks[masks>0] = 1 # 这里的mask只是为了指示背景,转screen space的时候背景会变色,要mask回去
        ...
        normals = torch.matmul(normals, R) # b h w 1 k 利用camera转成screen space
        normals = rearrange(normals, "b h w 1 k -> b k h w")
        # normals = torch.einsum('bchw,bij->bihw', normals, -self.R) # 前景在[-1,1] 且长度为1
        #     # images = images.squeeze(1)                            # 背景长度为sqrt3 可能超出[-1,1] 所以要mask
        normals = (normals + 1) / 2 # 前景到[0,1]
        normals = normals * masks
        
        if return_mask: # 多返回一个陡峭的mask
    ...
    @torch.no_grad()
    def render_semantic(self, bg="black"): # 不可微normal,screen_space,用于nvs条件
        # output:
        #     semantics: tensor B,C,H,W [0,1]
        images = self.get_image(type="semantic", bg=bg).detach() # B,C=4,H,W [0,1]范围 和渲染数据预处理的结果是对应的
        images = images[:,:3,:,:]
        return images