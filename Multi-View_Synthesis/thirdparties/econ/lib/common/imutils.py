...
def draw_landmarks_on_image(image, landmarks, radius=3, color=(255, 0, 0)):
    # 创建一个可绘制的图像对象
...
def get_pymafx(image, landmarks):
    ...
    for part in ['lhand', 'rhand', 'face']:
        ...
        # ##debug
        # img_pil = tensor_normalize_to_pil(img_part)
        # img_pil.save('debug.png')
...
def process_image(img_pil):
    ...
    img_crop = np.array(img_pil) # H,W,C=3 原图转tensor
    ...
    landmark_lst.append(landmarks['body']) # 33个点
    ...
    # img_np    (array): (0, 255),          [512,512,3]  img_crop去背景*mask 
    # img_rembg (array): (0, 255),          [512,512,4]
    # img_mask  (array): (0, 1),            [512,512,1]
    # img_crop  (array): (0, 255),          [512,512,3]

    return_dict = {
        "img_icon": torch.stack(img_icon_lst).float(),    #[N, 3, res, res]  img_np归一化到-1~1 且只保留了三通道 
        "img_crop": torch.stack(img_crop_lst).float(),    #[N, 3, res, res]  单人检测出来之后 resize 和扩充到512              
        "img_hps": torch.stack(img_hps_lst).float(),    #[N, 3, res, res]  img_np缩小到244
        "img_mask": torch.stack(img_mask_lst).float(),    #[N, res, res] 处理过的mask 去掉float
        "landmark": torch.stack(landmark_lst),    #[N, 33, 4]
        "hands_visibility": hands_visibility_lst, 
        # "img_rembg": torch.stack(img_rembg_lst).float(),    #[N, 4, res, res] 去背景的结果 没对mask做处理 可能有一些噪点
    }
    
    # 这里是画图验证一下这些图都是什么
    # names = ["img_icon", "img_crop", "img_hps"]
    # lmk = return_dict['landmark'].squeeze(0).detach().cpu().numpy()
    # for k, v in return_dict.items():
    #     if k in names:
    #         v = tensor_normalize_to_pil(v)
    #         v = draw_landmarks_on_image(v, lmk)
    #         v.save(f"vis/{k}.png")    
...
def process_video(video_np, normal_np):
    ...
    img_pymafx_lst = []
    img_rembg_lst = []
    video_np = (video_np * 255).astype('uint8') # 和image process对齐 先变成255
    n_frames = video_np.shape[0]
    for idx in range(n_frames):
        img_crop = video_np[idx]
        # img_crop H,W,C=3 原图转tensor
        ...
        img_rembg_lst.append(torch.tensor(img_rembg).permute(2, 0, 1) / 255.0)
        ##debug
        xxx=tensor_normalize_to_pil(img_icon)
        xxx = draw_landmarks_on_image(xxx, landmarks['body'])
        xxx.save(f"vis/debug.png")
    ...
    return_dict = {
        "img_icon": torch.stack(img_icon_lst).float(),    #[N, 3, res, res] [-1,1] img_np归一化到-1~1 且只保留了三通道 
        "img_crop": torch.stack(img_crop_lst).float(),    #[N, 3, res, res]  [0,1] 单人检测出来之后 resize 和扩充到512              
        "img_hps": torch.stack(img_hps_lst).float(),    #[N, 3, res, res]  img_np缩小到244
        "img_mask": torch.stack(img_mask_lst).float(),    #[N, res, res] [0,1] 处理过的mask 去掉float
        "landmark": torch.stack(landmark_lst),    #[N, 33, 4] [-1,1]
        "img_normal": torch.stack(img_normal_lst).float(),    #[N, 3, res, res] 返回的normal [0,1]
        "hands_visibility": hands_visibility_lst, 
        # "img_rembg": torch.stack(img_rembg_lst).float(),    #[N, 4, res, res] 去背景的结果 没对mask做处理 可能有一些噪点
    }
