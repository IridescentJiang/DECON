...
def load_human_nvs_results(mv_dir, ref_path, imSize, view_num=20):
    ...
    ## 再加入生成结果
    for idx in range(1, view_num): # 正面用真实ref
        azim = idx * 360 // view_num
        elev = 0.0
        rgb_filepath = os.path.join(mv_dir, f"{azim:03d}.png")
        # print(rgb_filepath)
        image =np.array(Image.open(rgb_filepath).resize(imSize))[:, :, :3] # 白底
        image_masked = remove(image) # 黑底？
        mask = image_masked[:, :, 3]
        # # color mask是用rembg的mask & 像素接近白色的点 防止去掉太多背景
        # color_mask = image_masked[:, :, 3]
        ...
        all_images.append(image)
        all_masks.append(mask)
        all_azims.append(azim * 1.0) # 转float
        all_elevs.append(elev)

    # return np.stack(all_images), np.stack(all_color_masks), np.stack(all_azims), np.stack(all_elevs)
...
class TexturedMeshRenderer:
    ...
    def render_mesh(self, bg="black", return_mask=True): # 可微normal,world_space,用于和branch分支算loss
        # output:
        #     normals: tensor B,C=3,H,W [0,1]
        images = self.get_image(type="rgb", bg=bg) # world B,C=4,H,W [0,1]范围 和渲染数据预处理的结果是对应的 
        mask = images[:, 3, :, :] > 0.5
        images = images[:, :3, :, :]
