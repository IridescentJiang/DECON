...
def apply_vertex_mask(mesh, vertex_mask):
    ...
    faces_mask = vertex_mask[mesh.faces].any(dim=1) # 只要有vert是手部 face就是手部
...
def poisson(mesh, obj_path, depth=10, decimation=True):
    ...
    largest_mesh = keep_largest(trimesh.Trimesh(np.array(mesh.vertices), np.array(mesh.triangles), vertex_colors=np.array(mesh.vertex_colors)))
