...
def vis_meshes_img_depth(img, verts, smpl_faces, cam_intrinsics, colors=None, padding=True):
    if not isinstance(img, np.ndarray):
        img = tensor_to_BGR(img.detach().cpu())
    ...
    depthanything_min_depth = 0.1  # 假设的最小深度值，根据需求调整
    depthanything_max_depth = 5.0  # 假设的最大深度值，根据需求调整

    if len(cam_intrinsics.flatten()) == 9:
        cam_intrinsics = cam_intrinsics.reshape(3, 3)
        rgb, depth = render_mesh(img.shape[0], img.shape[1], verts, smpl_faces, cam_intrinsics, None)
        
        # Clip depth to be within the specified range
        clipped_depth = np.clip(depth, depthanything_min_depth, depthanything_max_depth)
        
        # Normalize depth between 0 and 1 within this expected range
        normalized_depth = (clipped_depth - depthanything_min_depth) / (depthanything_max_depth - depthanything_min_depth)
        
        # Convert normalized depth to grayscale
        depth_img = (1 - normalized_depth) * 255
        depth_img = np.clip(depth_img, 0, 255).astype(np.uint8)
        
        # Create a 3-channel image for visualization
        rendered_img = np.zeros_like(img)  # Start with a black image background
        rendered_img[depth > 0] = np.stack([depth_img] * 3, axis=-1)[depth > 0]

    else:
        rendered_img = np.zeros_like(img)  # Start with a black image background
        for i, cam_int in enumerate(cam_intrinsics):
            rgb, depth = render_mesh(img.shape[0], img.shape[1], [verts[i]], smpl_faces, cam_int, None)
            
            # Clip and normalize depth for each view
            clipped_depth = np.clip(depth, depthanything_min_depth, depthanything_max_depth)
            normalized_depth = (clipped_depth - depthanything_min_depth) / (depthanything_max_depth - depthanything_min_depth)
            depth_img = (1 - normalized_depth) * 255
            depth_img = np.clip(depth_img, 0, 255).astype(np.uint8)
            
            depth_img_rgb = np.stack([depth_img] * 3, axis=-1)
            valid_mask = (depth > 0)[:, :, None]
            rendered_img[valid_mask] = (
                depth_img_rgb * valid_mask +
...
def output_meshes(meshes, face):
    # 创建一个空的 Trimesh 对象，用于合并所有的 mesh
    combined_mesh = trimesh.Trimesh()

    for i, mesh_verts in enumerate(meshes):
        # 创建一个新的 Trimesh 对象
        mesh = trimesh.Trimesh(vertices=mesh_verts, faces=face)
        
        # 应用旋转，使得 mesh 在空间中有正确的方向
        rot = trimesh.transformations.rotation_matrix(np.radians(180), [1, 0, 0])
        mesh.apply_transform(rot)
        
        # 合并当前 mesh 到总的 combined_mesh
...
def vis_points(img, points, color=(0, 0, 255), radius=5, thickness=2):
    if not isinstance(img, np.ndarray):
        img = tensor_to_BGR(img.detach().cpu())

    # 绘制每个点
    for point in points:
        point = point.astype(int)  # 转换为整数坐标
        cv2.circle(img, (point[0], point[1]), radius, color, thickness)  # 绘制圆点
