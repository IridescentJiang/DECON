...
class TetraSMPL(nn.Module):
    ...
    def __init__(self, model_file, model_additional_file):
        ...
        i = torch.LongTensor(indices.astype(np.int64))
    ...
    def forward(self, pose, beta):
        ...
        beta = beta[:, :, None].to(shapedirs.device)
        ...
        # input it rotmat: (bs,72)
            ...
            print("pose.ndimension is invalid!")
        I_cube = torch.eye(3)[None, None, :].to(self.device)
        ...
        pad_row = torch.FloatTensor([0,0,0,1]).to(self.device).view(1,1,1,4).expand(batch_size, 24, -1, -1)
        ...
        rest = torch.cat([J, torch.zeros(batch_size, 24, 1).to(self.device)], dim=2).view(batch_size, 24, 4, 1)
        zeros = torch.zeros(batch_size, 24, 4, 3).to(self.device)
    ...
    def get_vertex_transformation(self, pose, beta):
        ...
        J_[:, 1:, :] = J[:, 1:, :] - J[:, self.parent, :]
        G_ = torch.cat([R, J_[:, :, :, None]], dim=-1)
        pad_row = torch.FloatTensor([0, 0, 0, 1]).to(self.device).view(1, 1, 1, 4).expand(batch_size, 24,
                                                                                     -1, -1)
        G_ = torch.cat([G_, pad_row], dim=2)
        G = [G_[:, 0].clone()]
        for i in range(1, 24):
    ...
    def rot_vert(self, vs, transl):
        ...
        rotation_matrix_y = torch.tensor([[-1, 0, 0],
                                          [0, 1, 0],
                                          [0, 0, -1]], device=self.device, dtype=torch.float32)

        vs = torch.matmul(vs, rotation_matrix_y)

        rotation_matrix_z = torch.tensor([[-1, 0, 0],
                                          [0, -1, 0],
                                          [0, 0, 1]], device=self.device, dtype=torch.float32)

        vs = torch.matmul(vs, rotation_matrix_z)
    ...
    def transl_respectively(self, vs, transl=None):
        # the transl must be normalized by scale firstly
        if transl is None:
            transl = [0, 0, 0]
        vs += transl
        return vs
...
# 将Thuman2.0提供的GT smpl的pkl文件转化为可四面体化的smpl模型（非批量）
if __name__ == '__main__':
    ...
    if file_path.endswith('.pkl') and os.path.isfile(file_path):
        ...
        pose = torch.Tensor(body_pose).float()
        shape = torch.Tensor(betas).float()
        ...
        # 拼接文件路径
        filepath = os.path.join(filepath, "smpl_0.obj")
