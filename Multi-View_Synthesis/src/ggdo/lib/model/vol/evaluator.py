...
class Evaluator(object):
    def __init__(self, device):
        ...
        # chose "Perspective" when in_the_wild inference
        # self.render_type = "Perspective"
        self.render_type = "Orthographic"
    ...
    def optm_smpl_param_depth(self, pose_transl, shape, iter_num, refer_depth_map, nmlF, rotation_matrix, mask, name,
        ...
        for i in pbar_mask:
            ...
            if self.render_type == "Orthographic":
                with torch.no_grad():
    ...
    def optm_smpl_param_shrink(self, img, pose, shape, iter_num, calib=None, nmlF=None, nmlB=None, current_depth_map=None,
        ...
        for i in tqdm(range(iter_num), desc='Body Fitting Optimization'):
            ...
            if i < iter_num * (1 / 2):
                ...
                theta_new_ = torch.cat([theta_new[:, :39], theta_orig[:, 39:45], theta_new[:, 45:48], theta_orig[:, 48:60], theta_new[:, 60:75]], dim=1)
    ...
    def rendering_smpl_mask(self, verts, faces, scale):
        ...
        # self.render.load_meshes(verts, faces)
        F_mask, B_mask = self.render.get_image(type="mask", scale=scale, render_type=self.render_type)
