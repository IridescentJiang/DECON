...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        if self.config.grid_prune: # ture
            self.occupancy_grid = OccupancyGrid( # 128*128*128的网格点 并且告诉了边界是-1,1
        ...
        self.randomized = self.config.randomized # True
    ...
    def forward_(self, rays):
        ...
        if self.config.geometry.grad_type == 'finite_difference':
            ...
            sdf, sdf_grad, feature = self.geometry(positions, with_grad=True, with_feature=True) # 过网络得到采样点的值

        normal = F.normalize(sdf_grad, p=2, dim=-1)
        alpha = self.get_alpha(sdf, normal, t_dirs, dists)[...,None]
        rgb = self.texture(feature, t_dirs, normal) # 过网络得到采样点的值

        weights = render_weight_from_alpha(alpha, ray_indices=ray_indices, n_rays=n_rays)
        opacity = accumulate_along_rays(weights, ray_indices, values=None, n_rays=n_rays) # 什么都不累积 是不透明度
        depth = accumulate_along_rays(weights, ray_indices, values=midpoints, n_rays=n_rays) # 中点累积 深度
        comp_rgb = accumulate_along_rays(weights, ray_indices, values=rgb, n_rays=n_rays) # rgb累积 颜色

        comp_normal = accumulate_along_rays(weights, ray_indices, values=normal, n_rays=n_rays) # normal累积 法线
        comp_normal = F.normalize(comp_normal, p=2, dim=-1) # 归一化normal

        pts_random = torch.rand([1024*2, 3]).to(sdf.dtype).to(sdf.device) * 2 - 1  # normalized to (-1, 1)

        if self.config.geometry.grad_type == 'finite_difference': # 有限差分是在random点上加扰动，获得grad(normal perturb)
    ...
    def export(self, export_config):
        ...
        if export_config.export_vertex_color:
            _, sdf_grad, feature = chunk_batch(self.geometry, export_config.chunk_size, False, mesh_colored['v_pos'].to(self.rank), with_grad=True, with_feature=True)
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            rgb = self.texture(feature, -normal, normal) # set the viewing directions to the normal to get "albedo"
            mesh_colored['v_rgb'] = rgb.cpu()
        return mesh, mesh_colored
