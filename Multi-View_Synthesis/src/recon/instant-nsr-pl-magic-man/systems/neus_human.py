...
@systems.register('human-neus-system')
class OrthoNeuSSystem(BaseSystem):
    ...
    def prepare(self):
        ...
        self.train_num_samples = self.config.model.train_num_rays * (self.config.model.num_samples_per_ray + self.config.model.get('num_samples_per_ray_bg', 0)) # 256 * 1024
        self.train_num_rays = self.config.model.train_num_rays # 256
        self.cos = torch.nn.CosineSimilarity(dim=-1, eps=1e-6)

    def forward(self, batch):
        return self.model(batch['rays'])
    
    def preprocess_data(self, batch, stage): # 把图片转换成光线对应的采样点
        if 'index' in batch: # validation / testing
            index = batch['index']
        else:
            if self.config.model.batch_image_sampling: # true 从图片序号中取256个 分别取一条ray 共取256条
        ...
        if stage in ['train']:
            ...
            elif self.dataset.directions.ndim == 4: # (N, H, W, 3) Here 正交相机 全部都是0,0,1
                directions = self.dataset.directions[index, y, x] # 256*3 方向全是0,0,1
                origins = self.dataset.origins[index, y, x] # 从6(张一样的)*1024*1024*3的网格坐标中选择y,x位置的(xy在-1~1,z都是0)
            rays_o, rays_d = get_ortho_rays(origins, directions, c2w) # o是相机中心位置+坐标位置 d是相机在世界坐标的方向
            rgb = self.dataset.all_images[index, y, x].view(-1, self.dataset.all_images.shape[-1]).to(self.rank) # 256*3 像素rgb真值
            normal = self.dataset.all_normals_world[index, y, x].view(-1, self.dataset.all_normals_world.shape[-1]).to(self.rank) # 像素normal真值
            fg_mask = self.dataset.all_fg_masks[index, y, x].view(-1).to(self.rank) # 像素mask
            rgb_mask = self.dataset.all_rgb_masks[index, y, x].view(-1).to(self.rank) # 
            view_weights = self.dataset.view_weights[index, y, x].view(-1).to(self.rank) # 每张图权重
        else:
        ...
        cosines = self.cos(rays_d, normal) # 图片像素的normal和光线方向的cos
        ...
        if self.dataset.apply_mask:
            rgb = rgb * fg_mask[...,None] + self.model.background_color * (1 - fg_mask[...,None]) # 这里用normal mask把rgb背景变黑？？
    ...
    def training_step(self, batch, batch_idx):
        ...
        cosines = batch['cosines'] # 光线和mv normal的夹角
        ...
        ## RGB loss 0.5*MSE+0*L1
        erros_rgb_mse = F.mse_loss(out['comp_rgb_full'][rgb_mask], batch['rgb'][rgb_mask], reduction='none') # 256*3取mask为true的一部分 所以实际不到256
        ...
        loss_rgb_mse = ranking_loss(erros_rgb_mse.sum(dim=1), 
                                    penalize_ratio=self.config.system.loss.rgb_p_ratio, type='mean') # 保留前0.8
        ...
        ## normal loss 会用光线和normal夹角加权 用Cos Loss 权重1.0
        ...
        if self.config.system.loss.geo_aware:   # here
            normal_errors = normal_errors * torch.exp(cosines.abs()) / torch.exp(cosines.abs()).sum()
            loss_normal = ranking_loss(normal_errors[mask], # 都只取mask部分 (这里的mask是normal maks & cos<0.1)
        ...
        ## eik正则化 让梯度normal接近长度1
        loss_eikonal = ((torch.linalg.norm(out['sdf_grad_samples'], ord=2, dim=-1) - 1.)**2).mean() 
        self.log('train/loss_eikonal', loss_eikonal, prog_bar=True, rank_zero_only=True)
        loss += loss_eikonal * self.C(self.config.system.loss.lambda_eikonal)
        ## mask loss (不透明度算mask) 权重1 同样取top80
        opacity = torch.clamp(out['opacity'].squeeze(-1), 1.e-3, 1.-1.e-3) # 0~1
        ...
        ## 稀疏化loss 关于sdf的绝对值递减 所以希望sdf绝对值尽量大 远离0 就是空间中大部分值都不是0表面 防止出现额外漂浮的表面
        ...
        if self.C(self.config.system.loss.lambda_distortion) > 0: # false
            loss_distortion = flatten_eff_distloss(out['weights'], out['points'], out['intervals'], out['ray_indices'])
            self.log('train/loss_distortion', loss_distortion)
            loss += loss_distortion * self.C(self.config.system.loss.lambda_distortion)    

        if self.config.model.learned_background and self.C(self.config.system.loss.lambda_distortion_bg) > 0: # false
        ...
        ## 光滑loss 让sdf表面光滑 让随机点的normal和随机点周围扰动的normal接近
    ...
    def validation_epoch_end(self, out):
        ...
        self.export()  # 这里导出mesh 调用neus(model)的export 再调用volume-sdf(geometry)的isosurface
    ...
    def export(self):
        mesh, mesh_colored = self.model.export(self.config.export)
        # pdb.set_trace()
        self.save_mesh(
        ...
        if self.config.export.export_vertex_color:
            self.save_mesh(
                f"it{self.global_step}-{self.config.model.geometry.isosurface.method}{self.config.model.geometry.isosurface.resolution}-colored.obj",
                ortho_scale=self.config.export.ortho_scale,
                **mesh_colored
            )
    
    def on_fit_start(self):
        update_module_step(self.model, 0, 0)
        # initialize SDF
        self.model.geometry.initialize_shape(run_init=self.config.system.run_initial)
