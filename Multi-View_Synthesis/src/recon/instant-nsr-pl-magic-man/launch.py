...
def main():
    ...
    config = load_config(args.config, cli_args=extras) # 在这里合并了dataset的root和scene
    config.cmd_args = vars(args)

    # config.trial_name = config.get('trial_name') or (config.tag + datetime.now().strftime('@%Y%m%d-%H%M%S'))
    # config.exp_dir = config.get('exp_dir') or os.path.join(args.exp_dir, config.name) # exp/0000
    # config.save_dir = config.get('save_dir') or os.path.join(config.exp_dir, config.trial_name, 'save') # exp/0000/@2024/save
    ...
    config.trial_name = config.get('trial_name')
    # config.exp_dir = config.get('exp_dir') or os.path.join(config.dataset.mv_dir, "neus") # test_set/0000/mv/gt_gen/nvs_dual_branch@step2_union_v1/neus
    config.exp_dir = os.path.join(config.dataset.mv_dir, "neus") # test_set/0000/mv/gt_gen/nvs_dual_branch@step2_union_v1/neus
    # BUG:!!!!!要改 yaml里面的exp_dir也要注释掉
    config.save_dir = os.path.join(config.exp_dir, 'save') # neus/save
    config.ckpt_dir = os.path.join(config.exp_dir, 'ckpt')
    # config.code_dir = config.get('code_dir') or os.path.join(config.exp_dir, config.trial_name, 'code')
    config.config_dir = os.path.join(config.exp_dir, 'config')

    print(config.trial_name)
    ...
    system = systems.make(config.system.name, config, load_from_checkpoint=None if not args.resume_weights_only else args.resume) # 如果weight only 那么这里就用了resume的ckpt
                                                                                                # 否则在fit的时候用ckpt
    callbacks = []
    if args.train:
        callbacks += [
    ...
    if args.train:
        loggers += [
            ...
            CSVLogger(config.exp_dir, name="", version='csv_logs')
        ]
...
#  python launch.py --config configs/human-nvs.yaml --gpu 0 --train