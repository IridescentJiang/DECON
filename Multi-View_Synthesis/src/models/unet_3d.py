...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        use_inflated_groupnorm=False, # stage1是false stage2是true(多帧情况)
        # Additional
        ...
        for i, (down_block_type, down_block_attention_index) in enumerate(zip(down_block_types,down_block_attention_indices)):
            ...
            # 这里和reference net不一样，get的block是3d的，但是stage1中，use_motion_module设置为false
    ...
    def forward(
        ...
        pose_cond_fea: Optional[torch.Tensor] = None, # master的smpl pose条件
        pose_cond_fea_list: Optional[List[torch.Tensor]] = None, # branch的smpl pose条件
        ...
        r"""
        Args:
            sample (`torch.FloatTensor`): (batch, channel, frame, height, width) noisy inputs tensor       噪声采样，输入已经包含了时间维度
            timestep (`torch.FloatTensor` or `float` or `int`): (batch) timesteps
            encoder_hidden_states (`torch.FloatTensor`): (batch, sequence_length, feature_dim) encoder hidden states    这里是clip enc的结果
        ...
        # camera_emb
        camera = camera.expand(sample.shape[0], -1, -1) # 这是在batch维度扩展camera 在推理阶段便于cfg batch内多个sample只给了一个t
        camera_emb = self.camera_embedding(camera) if camera is not None else 0 # b f c=1280
        # emb = emb + camera_emb # camera_emb 加给了t_emb
        ...
        if len(pose_cond_fea_list) > 0: # 加上condition条件 如果不用smpl的时候list是[None, ...]而不能是全空
            for i in range(self.branch_num):
        ...
        ## 分开走branch的后几层up (要先走branch 因为从sample copy,如果先走master的话sample就变了）
        sample_list = []
        for j in range(self.branch_num):
    ...
    def from_pretrained_2d(
        ...
        load_pe=True, # 原本的pe是一个buffer.训练时:用vpe的时候True 用rpe或者不用pe的时候不加载;推理的时候要加载
    ):
        ...
        # 加载SD unet的权重，其中复制一份norm1/attn1给norm0/attn0
        ...
        # 初始化attn0和norm0
        extra_state_dict = {} # attn0的参数用attn1即selfattn初始化 norm同理
        for k in state_dict:
        ...
        # load the motion module weights 加载motion module
        if motion_module_path.exists() and motion_module_path.is_file():
            ...
            if load_pe: # 用Vanilla pe的时候
                motion_state_dict = motion_state_dict
            else: # 用RPE或者不用PE的时候,把包含'pe'的项删掉
        ...
        # 用master初始化branch 这时候branch中也插入了motion module(最后进行 相当于master内部attn0 mm都已经填好了)
        branch_state_dict = {}
        for name, param in model.named_parameters():
            if ('branch' in name 
                and 'camera' not in name 
                and 'relative' not in name): # 初始化branch的参数（排除camera_emb_proj和relative pe的 因为这俩在master中也没有预训练初始化
                ###BUG:这里默认mm的zero_proj_out=false,所以master的state_dict中有proj_out的权重,可以用来初始化branch。True的情况没考虑
