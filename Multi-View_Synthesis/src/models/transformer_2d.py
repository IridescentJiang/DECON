...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # 3. Define transformers blocks selfatt和crossarr，实际上里面没用到timestep
    ...
    def forward(
        ...
        if not self.use_linear_projection: # here
            ...
            inner_dim = hidden_states.shape[1] # b, c, h, w
        ...
        # 2. Blocks
        if self.caption_projection is not None: # pass
        ...
        ref_feature = hidden_states.reshape(batch, height, width, inner_dim) # # b, c, h, w
        for block in self.transformer_blocks:  # BasicTransformerBlock
            if self.training and self.gradient_checkpointing: 
                ...
                hidden_states = block(
                    ...
                    timestep=timestep, # att中没用到timestep 是none
        ...
        if not return_dict: # here 返回在att之前的特征图作为ref_feature，以及最终输出output
