...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        # time
        if time_embedding_type == "fourier": # pass
        ...
        elif time_embedding_type == "positional": # t用位置编码
        ...
        # time_proj把时间映射到320，timestepembedding把320映射到1280
        ...
        if time_embedding_act_fn is None: # 走这里
        ...
        if isinstance(only_cross_attention, bool):
            if mid_block_only_cross_attention is None: # 走这里
                mid_block_only_cross_attention = only_cross_attention # false 也就是not only cross，还有self att
        ...
        if isinstance(num_attention_heads, int): # 8头
        ...
        if isinstance(layers_per_block, int):
            layers_per_block = [layers_per_block] * len(down_block_types) # [2,2,2,2]

        if isinstance(transformer_layers_per_block, int): # [1,1,1,1]
        ...
        if class_embeddings_concat:
            ...
            blocks_time_embed_dim = time_embed_dim # 1280 只有time_emb，没有和其他拼接
        ...
        # mid
        if mid_block_type == "UNetMidBlock2DCrossAttn": # 走这里
        ...
        reversed_layers_per_block = list(reversed(layers_per_block)) # 每个block 2层[resnet + transformer] 
        ...
        # if attention_type in ["gated", "gated-text-image"]: # 'default' pass
        #     positive_len = 768
    ...
    def forward(
        ...
        sample_noisy_list: List[torch.FloatTensor], ### branch输入
        timestep: Union[torch.Tensor, float, int],
        encoder_hidden_states: torch.Tensor,
        camera: Optional[torch.Tensor] = None, # R矩阵展开成9维 
        ...
        if encoder_attention_mask is not None: # false
        ...
        elif len(timesteps.shape) == 0: # false
            timesteps = timesteps[None].to(sample.device)

        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        timesteps = timesteps.expand(sample.shape[0]) # bs,4,64,64
        ...
        # camera_emb
        camera = camera.expand(sample.shape[0], -1) # 这是在batch维度扩展camera 在推理阶段便于cfg batch内多个sample只给了一个t
        camera_emb = self.camera_embedding(camera) if camera is not None else 0 # b c=1280 暂时每个batch内只有一张参考图（一个相机） 所以没有f维度
        ...
        # if self.config.addition_embed_type == "text": # false
        ...
        # elif self.config.addition_embed_type == "image": # false
        #     # Kandinsky 2.2 - style
        #     if "image_embeds" not in added_cond_kwargs:
        #         raise ValueError(
        ...
        # elif self.config.addition_embed_type == "image_hint": # false
        #     # Kandinsky 2.2 - style
        #     if (
        ...
        emb = emb + aug_emb if aug_emb is not None else emb # time emb
        ...
        ### 修改了down_block 先走前几个 和branch融合后走后面的
        ## down_block master的前几层 得到中间sample 和用于concat的res_sample
        ...
        for downsample_block in self.down_blocks[:self.copy_first_n_block]:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                sample, res_samples = downsample_block(
                    hidden_states=sample, # sample就是经过每个downblock得到的特征图
                    temb=emb, # time emb
                ...
                sample, res_samples = downsample_block(
                    hidden_states=sample, temb=emb, camera_emb=camera_emb, scale=lora_scale
                )
        ...
        ## 先把branch conv_in的结果放进res
        down_block_res_samples_list = []
        for i in range(self.branch_num): 
            down_block_res_samples_list.append((sample_in_list[i],))
        
        ## down_block branch的前几层 得到中间sample 和用于concat的res_sample
            ...
            for i, downsample_block in enumerate(self.down_blocks_branch[j]):
                if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                    ...
                    sample_in_list[j], res_samples = downsample_block(
                        ...
                        encoder_attention_mask=encoder_attention_mask,
                        **additional_residuals,
                    ) # sample_in_list[j]是每次block最后的输出;res_samples是block中间保存的结果,存入down_block_res_samples_list用于concat
                else:
                    sample_in_list[j], res_samples = downsample_block(
                        hidden_states=sample_in_list[j], temb=emb, camera_emb=camera_emb, scale=lora_scale
                    )
        ...
        ## 融合master和branch的down_block最终输出
        sample_list = [sample]
        for i in range(self.branch_num):
        ...
        for downsample_block in self.down_blocks[self.copy_first_n_block:]:
            ...
            down_block_res_samples += res_samples
            for i in range(self.branch_num): # 分支的res_sample把中间共享master的也存进去
        ...
        if self.mid_block is not None:
            if (
                ...
                sample = self.mid_block(sample, emb, camera_emb=camera_emb,)
        ...
        for i, upsample_block in enumerate(self.up_blocks[:-self.copy_last_n_block]):
            ...
            res_samples = down_block_res_samples[-len(upsample_block.resnets) :] # 该层要用的res concat
            down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)] # 剩余的res concat (pop)
            for j in range(self.branch_num):
                down_block_res_samples_list[j] = down_block_res_samples_list[j][: -len(upsample_block.resnets)] # branch只要算剩余的res concat
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample = upsample_block(
                    ...
                    temb=emb,
                    camera_emb=camera_emb,
                    res_hidden_states_tuple=res_samples, # skip连接
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
        ...
        if not return_dict: # here 只返回最后结果
            return (sample,)

        return UNet2DConditionOutput(sample=sample)


# refnet前传过程中，basicTransformer块的forward被重定义。保存了一些中间结果，用于ref feat
    ...
    def from_pretrained_2d(
        ...
        unet_config = cls.load_config(config_file) # config不用额外修改，因为attn0 attn1用的都是现有的config进行定义
        model = cls.from_config(unet_config, **unet_additional_kwargs) # 额外加入了一些config 如camera_dim等
        ...
        # 初始化attn0和norm0
        extra_state_dict = {} # attn0即view attn的参数用attn1即原本self attn(现在做ref attn)初始化 norm同理
        for k in state_dict:
        ...
        for name, param in model.named_parameters():
            if 'branch' in name and 'camera' not in name: # 初始化branch的参数（排除camera_emb_proj的 因为camera本来就没法用SD参数初始化）
                name_splits = name.split('.')
                master_name = '.'.join([name_splits[0]]+name_splits[2:]).replace("_branch", "") # 先把分支编号和branch去掉
                                                                                                # 例如'up_blocks_branch.0.0.attentions.0.norm.bias' -> 'up_blocks.0.attentions.0.norm.bias'
                if 'up_blocks' in name: # up_blocks的顺序要重新对应
                    # [0,1,2,3]
                    # [      0] 复制一层是这么对应
                    # [    0,1] 复制两层是这么对应
                    # [  0,1,2] 复制三层是这么对应
                    ...
                    master_name = '.'.join([name_splits[0], str(block_id_in_master)]+name_splits[2:]) # 例如'up_blocks.0.attentions.0.norm.bias' -> 'up_blocks.3.attentions.0.norm.bias'
                # 除了up_blocks之外，branch和master的顺序都是对好的，可以直接用参数
        ...
        m, u = model.load_state_dict(state_dict, strict=False) # camera相关参数是missing的 所以不能严格
