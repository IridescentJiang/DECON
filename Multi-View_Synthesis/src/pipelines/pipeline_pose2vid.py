...
class Pose2VideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Prepare a list of normal condition images
        if self.normal_guider is not None and normal_images is not None:
            normal_cond_tensor_list = []
            for normal_image in normal_images:
                normal_cond_tensor = (
                    torch.from_numpy(np.array(normal_image.resize((width, height)))) / 255.0 # RGB pil需要/255
                )
                normal_cond_tensor = normal_cond_tensor.permute(2, 0, 1).unsqueeze(
                    1
                )  # (c, 1, h, w)
                normal_cond_tensor_list.append(normal_cond_tensor)
            normal_cond_tensor = torch.cat(normal_cond_tensor_list, dim=1)  # (c, t, h, w)
            normal_cond_tensor = normal_cond_tensor.unsqueeze(0) # b c t h w
        ...
        if self.depth_guider is not None and depth_images is not None:
            depth_cond_tensor_list = []
            for depth_image in depth_images:
                depth_cond_tensor = (
                    torch.from_numpy(np.array(depth_image.resize((width, height)))) # 已经是0~1的pil 不用/255
                ) # h w 因为是单通道的所以这样算出来没有c
                depth_cond_tensor = depth_cond_tensor.unsqueeze(2) # h w c
                depth_cond_tensor = depth_cond_tensor.permute(2, 0, 1).unsqueeze(
                    1
                )  # (c=1, 1, h, w)
                depth_cond_tensor_list.append(depth_cond_tensor)
            depth_cond_tensor = torch.cat(depth_cond_tensor_list, dim=1)  # (c=1, t, h, w)
            depth_cond_tensor = depth_cond_tensor.unsqueeze(0) # b c=1 t h w
            depth_cond_tensor = depth_cond_tensor.repeat(1, 3, 1, 1, 1) # b c=3 t h w 转为3通道，之后才能过depth_guider
