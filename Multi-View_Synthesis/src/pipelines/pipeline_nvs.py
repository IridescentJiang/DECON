...
class NovelViewPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        unet_attention_mode="read_concat_view",  # 新增 ["read_single_view", "read_multi_view"]
    ...
    def __call__(
        ...
        num_inference_steps,
        guidance_scale,
        smplx_guidance_scale, # smplx占比
        guidance_rescale: float = 0.7, # 解决cfg过曝的rescale cfg7.5对应0.7,或许可以调小 
        ...
        **kwargs,
        ...
        ## 2. Prepare clip image embeds
        if self.image_encoder is not None: #
            clip_image = self.clip_image_processor.preprocess(
                ref_rgb_image, return_tensors="pt"
            ).pixel_values
        ...
        reference_control_reader = ReferenceAttentionControl(
            self.denoising_unet,
            do_classifier_free_guidance=do_classifier_free_guidance,
            mode=self.unet_attention_mode, # concat/cross 具体是single/multi view 由网络参数的indices控制
            batch_size=batch_size, # 1
            fusion_blocks="full",
        )

        ## 3. 准备噪声latents输入 包括rgb(master)和normal(branch list)
        ...
        latents = self.prepare_latents(
            ...
            height,
            video_length,
            self.vae.dtype, # 原本的clip_image_embeds.dtype可能不存在
            device,
            generator,
        )
        # branch
        ...
        ## 5. 准备cond semantic/normal
        if self.semantic_guider is not None and cond_semantic_images is not None: # 后者用于w/o smplx
            cond_semantic_tensor_list = []
            for cond_semantic_image in cond_semantic_images:
                cond_semantic_tensor = ( ## BUG: 确认PIL.resize
                    torch.from_numpy(np.array(cond_semantic_image.resize((width, height)))) / 255.0 # RGB pil需要/255
        ...
        if self.normal_guider is not None and cond_normal_images is not None:
            ...
            for cond_normal_image in cond_normal_images:
                cond_normal_tensor = (
                    torch.from_numpy(np.array(cond_normal_image.resize((width, height)))) / 255.0 # RGB pil需要/255
                ).permute(2, 0, 1).unsqueeze(1)  # (c, 1, h, w)
                cond_normal_tensor_list.append(cond_normal_tensor)
            cond_normal_tensor = torch.cat(cond_normal_tensor_list, dim=1).unsqueeze(0) # b c t h w
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t) #BUG: scale
                # branch
                ...
                # 每一步，同一套latents、ref、clip、t、camera分别用w/ smplx和w/o smplx都过一次，然后混合
                # 但是每次过的noise pred都先各自混合ref的cfg
                '''w/ smplx + cfg'''
                ...
                if do_classifier_free_guidance and guidance_rescale > 0.0: # 对混合的noise进一步rescale(根据cond结果)
                ...
                if do_classifier_free_guidance:
                    ...
                    noise_pred_wo_smplx = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond) # b=1,c=4,f,h=64,w=64
                if do_classifier_free_guidance and guidance_rescale > 0.0: # 对混合的noise进一步rescale(根据cond结果)
                    # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                    noise_pred_wo_smplx = rescale_noise_cfg(noise_pred_wo_smplx, noise_pred_text, guidance_rescale=guidance_rescale)
                    
                '''混合w/ w/o smplx 也是一种cfg'''
                ...
                '''做完所有cfg后把branch和master拆开'''
                noise_pred_list = [noise_pred[:, 4:8]] # branch
                noise_pred = noise_pred[:, :4] # master后修改 要不然branch取不了
                
                ''' 根据混合的估计噪声对latents做一步去噪 x_t -> x_t-1 '''
                latents = self.scheduler.step( # master
                    noise_pred, t, latents, **extra_step_kwargs, return_dict=False
                )[0]
                assert len(latents_list) == len(noise_pred_list) == self.denoising_unet.branch_num
                for idx in range(self.denoising_unet.branch_num): # branch循环
        ...
        # 修改了decode_latents函数 对rgb和normal分别decode
        ...
        return NovelViewPipelineOutput(rgb_videos=rgb_images, normal_videos=normal_images)
